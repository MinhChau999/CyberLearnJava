...
public class XuLy {
	...
	// Tìm phần tử chẵn cuối cùng
	public static void finalEvenNumber(int[][] a, int row, int column) {
		int finalEvenNumber = 0;
		for (int i = 0; i< row; i++) {
			for(int j =0; j< column; j++) {
				if(a[i][j] !=0 && a[i][j] % 2 == 0 ) {
					finalEvenNumber = a[i][j];
				}
			}
		}
		if(finalEvenNumber == 0) {
			System.out.println("Không có số chẵn trong mảng");
		} else {
			System.out.println("Số chẵn cuối cùng trong mảng: " + finalEvenNumber);
		}
	}
}
