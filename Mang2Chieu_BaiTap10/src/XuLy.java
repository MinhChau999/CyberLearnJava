...
public class XuLy {
	...
	// Liệt kê dòng chưa giá trị âm
	public static void rowNegativeValue(int[][] a, int row, int column) {
		boolean flag;
		boolean check = false;
		for (int i = 0; i < row; i++) {
			flag = false;
			...
			if (flag) {
				System.out.println("Dòng " + i + " chứa giá trị âm.");
			}
		}
		if (!check) {
			System.out.println("Mảng không chứa giá trị âm.");
		}
	}
}
